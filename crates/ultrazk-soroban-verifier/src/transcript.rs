...
//! Barretenberg `UltraZKFlavor` verifier (v0.87.0). Every challenge round,
...
//!
//! BB reference: Barretenberg v0.87.0 `transcript/transcript.hpp`
...
fn generate_libra_challenge(env: &Env, proof: &Proof, previous_challenge: Fr) -> (Fr, Fr) {
    let mut data = Bytes::new(env);
    data.extend_from_slice(&previous_challenge.to_bytes());
...
fn generate_rho_challenge(env: &Env, proof: &Proof, previous_challenge: Fr) -> (Fr, Fr) {
    ...
    data.extend_from_slice(&proof.libra_claimed_evaluation.to_bytes());
    push_point(&mut data, &proof.libra_grand_sum_commitment);
    push_point(&mut data, &proof.libra_quotient_commitment);
    push_point(&mut data, &proof.hiding_polynomial_commitment);
    data.extend_from_slice(&proof.hiding_polynomial_evaluation.to_bytes());
    let next_previous_challenge = hash_to_fr(&data);
...
fn generate_shplonk_nu_challenge(env: &Env, proof: &Proof, previous_challenge: Fr) -> (Fr, Fr) {
    ...
    for evaluation in proof.libra_polynomial_evaluations.iter() {
