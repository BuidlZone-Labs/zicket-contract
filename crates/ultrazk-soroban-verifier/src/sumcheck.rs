...
pub fn verify_sumcheck(
    ...
    let mut round_target = &tp.libra_challenge * &proof.libra_sum;
    ...
    for challenge in tp.sumcheck_u_challenges.iter().take(log_n).skip(2) {
        evaluation = evaluation * challenge;
    }
    let masked_relation_sum = grand_honk_relation_sum * (&one - evaluation)
        + &proof.libra_claimed_evaluation * &tp.libra_challenge;

    if masked_relation_sum == round_target {
        Ok(())
    } else {
        crate::trace!("===== SUMCHECK FINAL CHECK FAILED =====");
        crate::trace!(
            "grand_relation = 0x{}",
            crate::debug::Hex(&masked_relation_sum.to_bytes())
        );
        crate::trace!("target = 0x{}", crate::debug::Hex(&round_target.to_bytes()));
        crate::trace!(
            "difference = 0x{}",
            crate::debug::Hex(&(masked_relation_sum - round_target).to_bytes())
        );
        crate::trace!("======================================");
