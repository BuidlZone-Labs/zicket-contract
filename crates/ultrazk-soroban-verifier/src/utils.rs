...
const LIBRA_CONCATENATION_BYTES: usize = 128 + 32;
...
const ZK_OPENING_BYTES: usize = 32 + 3 * 128 + 32;
...
const LIBRA_EVAL_BYTES: usize = 4 * 32;
const FINAL_TWO_G1_BYTES: usize = 2 * 128;

const _: () = assert!(
    PAIRING_OBJ_BYTES
        + PROOF_HEAD_G1_BYTES
        + LIBRA_CONCATENATION_BYTES
        + SUMCHECK_UNIV_BYTES
        + SUMCHECK_EVAL_BYTES
        + ZK_OPENING_BYTES
...
pub fn load_proof(env: &Env, proof_bytes: &Bytes) -> Result<Proof, &'static str> {
    ...
    let libra_head = read_bytes::<LIBRA_CONCATENATION_BYTES>(proof_bytes, &mut boundary);
    let libra_concatenation_commitment =
        g1_from_proof_chunk128(env, libra_head[0..128].try_into().expect("libra concat"));
    let libra_sum = fr_word32(env, &libra_head, 4);
    ...
    let zk_opening = read_bytes::<ZK_OPENING_BYTES>(proof_bytes, &mut boundary);
    let libra_claimed_evaluation = fr_word32(env, &zk_opening, 0);
    let libra_grand_sum_commitment = g1_from_proof_chunk128(
        env,
        zk_opening[32..160].try_into().expect("libra grand sum"),
    );
    let libra_quotient_commitment = g1_from_proof_chunk128(
        env,
        zk_opening[160..288].try_into().expect("libra quotient"),
    );
    let hiding_polynomial_commitment = g1_from_proof_chunk128(
        env,
        zk_opening[288..416].try_into().expect("hiding polynomial"),
    );
    let hiding_polynomial_evaluation = fr_word32(env, &zk_opening, 13);
    ...
    let libra_evals = read_bytes::<LIBRA_EVAL_BYTES>(proof_bytes, &mut boundary);
    let libra_polynomial_evaluations = array::from_fn(|i| fr_word32(env, &libra_evals, i));
    ...
    Ok(Proof {
        ...
        hiding_polynomial_evaluation,
        gemini_fold_comms,
        gemini_a_evaluations,
        libra_polynomial_evaluations,
        shplonk_q,
        kzg_quotient,
    })
}
