...
fn public_inputs_are_canonical(env: &Env, public_inputs: &Bytes) -> bool {
    let mut offset = 0u32;
    while offset < public_inputs.len() {
        let mut encoded = [0u8; 32];
        public_inputs
            .slice(offset..offset + 32)
            .copy_into_slice(&mut encoded);
        if Fr::from_array(env, &encoded).to_bytes() != encoded {
            return false;
        }
        offset += 32;
    }
...
impl UltraHonkVerifier {
    ...
    /// Verify an UltraHonk ZK proof against the loaded VK.
    ///
    ...
    pub fn verify(
        ...
        if !public_inputs_are_canonical(env, public_inputs_bytes) {
            return Err(VerifyError::InvalidInput);
        }
...
#[cfg(test)]
mod tests {
    use super::public_inputs_are_canonical;
    use soroban_sdk::{Bytes, Env};

    #[test]
    fn rejects_scalar_modulus_as_non_canonical() {
        ...
        assert!(!public_inputs_are_canonical(
            &env,
            &Bytes::from_array(&env, &SCALAR_MODULUS)
        ));
    }
}
