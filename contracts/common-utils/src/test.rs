...
fn test_find_recipient_basis_points() {
    ...
    assert_eq!(
        validation::find_recipient_basis_points(&splits, &primary),
        Some(7000)
    );
    assert_eq!(
        validation::find_recipient_basis_points(&splits, &recipient2),
        Some(3000)
    );
    assert_eq!(
        validation::find_recipient_basis_points(&splits, &not_in_list),
        None
    );
}
